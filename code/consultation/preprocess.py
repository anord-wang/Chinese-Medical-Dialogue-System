...
def preprocess():
    ...
    parser.add_argument('--train_path', default='data/med_qa_train.txt', type=str, required=False, help='训练日志存放位置')
    parser.add_argument('--graph_input_path', default='data/med_Ga_train.txt', type=str, required=False, help='训练日志存放位置')
    parser.add_argument('--save_path', default='data/med_qa_graph_train.pkl', type=str, required=False, help='tokenize的训练数据集')
    ...
    with open(args.graph_input_path, 'rb') as f_graph:
        graph_data = f_graph.read().decode("gb18030", errors='ignore')
    ...
    if "\r\n" in graph_data:
        train_graph_data = graph_data.split("\r\n")
    else:
        train_graph_data = graph_data.split("\n")
    logger.info("there are {} dialogue in graph dataset".format(len(train_data)))
    ...
    with open(args.save_path, "w", encoding="utf-8") as f:
        for index, dialogue in enumerate(tqdm(train_data)):
            ...
            if index == 0:
                print(index)
            graph_utterance = train_graph_data[index]
            if graph_utterance == '您好，我是您的智能医疗助理，希望可以帮到您。如果没答上来，请更加详细的描述问题哦。祝您身体棒棒！':
                graph_utterance = ''
            # print(utterances)
            input_ids = [cls_id]  # 每个dialogue以[CLS]开头
            input_ids = input_ids + tokenizer.encode(utterances[0], add_special_tokens=False)
            input_ids = input_ids + tokenizer.encode(graph_utterance, add_special_tokens=False)
            input_ids.append(sep_id)
            input_ids = input_ids + tokenizer.encode(utterances[1], add_special_tokens=False)
            input_ids.append(sep_id)
